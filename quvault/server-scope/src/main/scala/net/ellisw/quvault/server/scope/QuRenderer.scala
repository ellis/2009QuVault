...
class QuRenderer(spec: QuRenderSpec, result: QuRenderResult, val state: QuRenderState) {
  private val enumerationLevelTypes = List("1", "A", "a", "i")

  def this(spec: QuRenderSpec) = this(spec, new QuRenderResult, new QuRenderState)
  
  def render: QuRenderResult = {
    result.xhtml = render(spec.xml.first, new QuRenderContext(null))
	result
  }
  ...
  private def handle_node(node: Node, ctx: QuRenderContext): Option[NodeSeq] = {
    ...
    val xhtml =
      node.label match {
        ...
        case "questionMathlib" => Some(new QuRendererMathlib(spec, result, state).render(node, new QuRenderContext(ctx)))
