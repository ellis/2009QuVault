...
public class Rect {
	...
	{
		// Initial values
		min = max = line.atAndOffPixels(nFraction, nLeftPadding);

		Point pt;
		pt = line.atAndOffPixels(nFraction, -nRightPadding);
		expand(pt);
		pt = line.atAndOnPixels(nFraction, nTopPadding);
		expand(pt);
		pt = line.atAndOnPixels(nFraction, -nBottomPadding);
		expand(pt);
	}
