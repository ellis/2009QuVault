...
public class Line {
	...
	public Point atAndOffPixels(double nFraction, double nPixelsOff) 
	{
		double dx = to.x - from.x;
		double dy = to.y - from.y;
	...
	/** Return the point that is at(nFraction) and off on the perpendicular vector by nPixelsOff */
	public Point atAndOffFraction(double nFraction, double nFractionOff) 
	{
		double dx = to.x - from.x;
		double dy = to.y - from.y;
		...
		if (r != 0)
		{
			pt.x -= dy * nFractionOff;
			pt.y += dx * nFractionOff;
		}

		return pt;
	}

	/** Return the point that is at(nFraction) and then backwards or forwards by nPixelsOn */
	public Point atAndOnPixels(double nFraction, double nPixelsOn) 
	{
		double dx = to.x - from.x;
		double dy = to.y - from.y;
