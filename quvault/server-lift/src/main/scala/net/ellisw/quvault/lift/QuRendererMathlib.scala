...
class QuRendererMathlib(spec: QuRenderSpec, state: QuRenderState) extends QuRenderer(spec, state) {
  ...
  private def transform_rhs_var(node: Node, find: String, ctx: QuRenderContext): Node = {
    ...
    if (!QuRequestVars.vars.contains(sFieldName))
      QuRequestVars.vars(sFieldName) = ""
    ...
    val xhtmlAnswer = spec.mode match {
      ...
      case QuAnswerMode.Edit => SHtml.text(QuRequestVars.vars(sFieldName), QuRequestVars.vars(sFieldName) = _, "maxlength" -> "80", "size" -> sEditCols)
  ...
  private def renderTextarea(node: Node, nFinds: Int): Node = {
    try {
      ...
      val sEditCols = Math.max(60, nAnswerCols).toString
      // REFACTOR: Somehow use specs.answers instead of QuRequestVars.vars -- ellis, 2009-06-04
